 ...
 */
function rtdustro_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'rtdustro_filter_comment_reply_link' );
 ...
 */
function rtdustro_twentytwenty_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
...
add_filter( 'edit_post_link', 'rtdustro_twentytwenty_edit_post_link', 10, 3 );
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				...
				if ( in_array( 'author', $post_meta, true ) ) {
					...
					<li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_attr_e( 'Post author', 'vaxi' ); ?></span>
				...
				if ( in_array( 'post-date', $post_meta, true ) ) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html_e( 'Post date', 'vaxi' ); ?></span>
				...
				if ( in_array( 'categories', $post_meta, true ) && has_category() ) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html_e( 'Categories', 'vaxi' ); ?></span>
							<?php twentytwenty_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
							<?php esc_html(_ex( 'In', 'A string that is output before one or more categories', 'vaxi' )); ?> <?php the_category( ', ' ); ?>
						</span>
					</li>
					<?php
				...
				if ( in_array( 'tags', $post_meta, true ) && has_tag() ) {
					...
					<li class="post-tags meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php esc_html_e( 'Tags', 'vaxi' ); ?></span>
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						...
						<span class="meta-text">
							<?php esc_html_e( 'Sticky post', 'vaxi' ); ?>
						</span>
					</li>
					<?php
...
function twentytwenty_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . esc_html__( 'Show sub menu', 'vaxi' ) . '</span>' . twentytwenty_get_theme_svg( 'chevron-down' ) . '</button>';
 ...
 */
function rtvaxi_get_the_archive_title( $title ) {
	$regex = apply_filters(
		'rtvaxi_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'rtvaxi_get_the_archive_title' );
